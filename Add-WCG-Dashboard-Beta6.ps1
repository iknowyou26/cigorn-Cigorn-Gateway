...
$backup = Join-Path $ProjectRoot "backup-dashboard-beta6-$stamp"
New-Item -ItemType Directory -Path $backup | Out-Null
Copy-Item $webserver, $webpages, $webpagesH $backup -Force
Write-Host "Backup created: $backup" -ForegroundColor Cyan

# ------------------------------------------------------------
# Add DashboardPage declaration to WebPages.h
# ------------------------------------------------------------
$h = Get-Content $webpagesH -Raw

if ($h -notmatch 'DashboardPage\s*\(') {
    $terminalDecl = [regex]::Match(
        $h,
        '(?m)^(?<indent>\s*)(?:std::)?string\s+TerminalPage\s*\(\s*(?:void)?\s*\)\s*;\s*$'
    )

    if (-not $terminalDecl.Success) {
        Fail "Could not locate TerminalPage declaration in WebPages.h"
    }

    $indent = $terminalDecl.Groups["indent"].Value
    $declaration = $terminalDecl.Value + "`r`n" +
                   $indent + "std::string DashboardPage();"

    $h = $h.Remove($terminalDecl.Index, $terminalDecl.Length)
    $h = $h.Insert($terminalDecl.Index, $declaration)

    Save-Utf8NoBom $webpagesH $h
    Write-Host "Added DashboardPage() declaration." -ForegroundColor Green
}
else {
...
# ------------------------------------------------------------
# Add DashboardPage implementation to WebPages.cpp
# ------------------------------------------------------------
...
'@
    ...
    $wp = $wp.Insert($terminalIndex, $dashboardFunction + "`r`n")
...
# ------------------------------------------------------------
# Add dashboard route to webserver.cpp
# ------------------------------------------------------------
$ws = Get-Content $webserver -Raw

if ($ws -notmatch 'PageRequested\s*==\s*"dashboard"') {
    $terminalRoute = [regex]::Match(
        $ws,
        '(?s)(?<indent>\s*)\}\s*else\s+if\s*\(\s*PageRequested\s*==\s*"terminal"\s*\)\s*\{.*?\n(?<bodyindent>\s*)\}'
    )

    if (-not $terminalRoute.Success) {
        Fail "Could not locate the terminal route in webserver.cpp"
    }

    $routeIndent = $terminalRoute.Groups["indent"].Value
    $bodyIndent = $terminalRoute.Groups["bodyindent"].Value

    $dashboardRoute =
        $terminalRoute.Value +
        "`r`n" + $routeIndent + "else if (PageRequested == `"dashboard`"){" +
        "`r`n" + $bodyIndent + "    webpage = MyPages.DashboardPage();" +
        "`r`n" + $bodyIndent + "    TimeSinceLastPage = 0;" +
        "`r`n" + $bodyIndent + "}"

    $ws = $ws.Remove($terminalRoute.Index, $terminalRoute.Length)
    $ws = $ws.Insert($terminalRoute.Index, $dashboardRoute)

    Save-Utf8NoBom $webserver $ws
    Write-Host "Added /?page=dashboard route." -ForegroundColor Green
}
else {
...
Write-Host "Building WCG Beta 6 dashboard..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Beta 6 dashboard build succeeded." -ForegroundColor Green
