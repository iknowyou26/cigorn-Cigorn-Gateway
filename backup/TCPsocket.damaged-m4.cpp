...
if (socketFlags < 0 || !(socketFlags & O_NONBLOCK))
{
    ...
    cigorn::PlatformLockGuard lock(qlock);
     ...
     if (txcount > MAXBUFSIZE){
         ...
         {
    cigorn::PlatformLockGuard lock(txbufflock);
         txcount = 0;
         rxbuff[0] = NUL;
       }
     }
...
bool tcpnet::initialize_socket(){
     ...
     if (newsockfd >= 0) {
    // Disable linger so close returns immediately.
    ...
    if (setsockopt(
            ...
            static_cast<int>(sizeof(yes))) == -1) {

        if (AttemptCount <= 1) {
            CoutM2(sockconsoleout)
                << "Failed to set reuse client socket on my port: "
                << portnum << endl;
        }
    }
    ...
    if (ioctlsocket(
            static_cast<SOCKET>(newsockfd),
            FIONBIO,
            &nonBlockingMode) != 0) {
#else
    int socketFlags = fcntl(newsockfd, F_GETFL, 0);

    if (socketFlags == -1 ||
        fcntl(newsockfd, F_SETFL, socketFlags | O_NONBLOCK) == -1) {
#endif
        DisconnectSocket();
    ...
    {
    cigorn::PlatformLockGuard lock(socklock);
...
{
    CigornShutdown(sockfd, SHUT_RDWR);
    if (CigornCloseSocket(sockfd);
    sockfd = -1;
    continue;
}
...
bool tcpnet::DisconnectSocket(void){
    ...
    if (newsockfd >= 0){
        CigornShutdown(newsockfd, SHUT_RDWR);
        usleep(CLOSEDELAY);
...
bool tcpnet::DisconnectSocket(void)
{
    if (newsockfd >= 0)
    {
        CigornShutdown(newsockfd);
        PlatformSleepMicroseconds(CLOSEDELAY);
...
bool tcpnet::sendtext(string s){
    ...
    while ((l > 0) && (txcount < (MAXBUFSIZE - 2)) && (txcount >= 0)){
       if (NLtoCRLF && (s[i] == NL)) {
           // We are supposed to translate '\r' into '\r' LF combo.
           txbuff[txcount++] = '\r';
           txbuff[txcount++] = NL;
       }else{
           txbuff[txcount++] = s[i];
